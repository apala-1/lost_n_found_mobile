...
debugPrint('📦 API models count: ${apiModels.length}');

final List<BatchEntity> entities = [];

for (final model in apiModels) {
  try {
    final entity = model.toEntity();
    entities.add(entity);
    debugPrint('✅ Entity added: ${entity.batchId}');
  } catch (e, stack) {
    debugPrint('❌ Entity mapping failed');
...
debugPrint('📊 Final entity count: ${entities.length}');
return Right(entities);
  ...
  debugPrint('❌ Repository unknown error');
  debugPrint('❌ Error: $e');
  debugPrint('❌ Stack: $stack');
